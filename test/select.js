...
export default ({test}) => {
    ...
    test('select builder: support subquery in the select clause without alias', t => {
        ...
        t.eq(actual, 'SELECT "foo", (SELECT * FROM "blah") FROM "bim"');
    });
    ...
    test('select builder: support subquery in the select clause with alias', t => {
        ...
        t.eq(actual, 'SELECT "foo", (SELECT * FROM "blah") AS "blah" FROM "bim"');
    });
    ...
    test('select builder: support subquery in the select clause with parameters', t => {
        ...
        t.eq(actual.text, 'SELECT "foo", (SELECT * FROM "blah" WHERE "param" = $1) AS "blah" FROM "bim" WHERE "params2" = $2');
        t.eq(actual.values, ['value1', 'value2']);
    });
    ...
    test('select builder: support query parameters', t => {
        ...
        t.eq(actual.text, 'SELECT * FROM "users" WHERE "foo" = $1 AND "blah" = $2');
        t.eq(actual.values, ['bar', 4]);
    });
    ...
    test('select builder: inner join', t => {
        ...
        t.eq(actual.text, 'SELECT * FROM "users" JOIN "products" ON "users"."id" = "products"."id" AND "users"."id" = 4');
    });
    ...
    test('select builder: join with subquery', t => {
        ...
        t.eq(actual.text, 'SELECT * FROM "users" JOIN (SELECT * FROM "products") ON "users"."id" = "products"."id" AND "users"."id" = 4');
    });
    ...
    test('select builder: join with params', t => {
        ...
        t.eq(actual.text, expected);
        t.eq(actual.values, [666, 5]);
    });
    ...
    test('select builder should be cloneable', t => {
        ...
        t.eq(s1.build().text, `SELECT "foo", "bar" AS "woot" FROM "users" WHERE "age" > 20 ORDER BY "age"`);
        t.eq(s2.build().text, `SELECT "foo", "bar" AS "woot" FROM "users" WHERE "age" > 20 ORDER BY "age" LIMIT 10`);
    });

    test('select builder: group by clause single column', t => {
        const q = select('foo', 'bar').from('woo').groupBy('foo').build();
        t.eq(q.text, `SELECT "foo", "bar" FROM "woo" GROUP BY "foo"`);
    });
    ...
    test('select builder: group by clause with multiple columns', t => {
        ...
        t.eq(q.text, `SELECT "foo", "bar" FROM "woo" GROUP BY "foo", "bar"."bim", "woot"`);
    });
    ...
    test('select builder: group by clause with having clause', t => {
        const q = select('foo', 'bar')
            ...
            .build();
        t.eq(q.text, `SELECT "foo", "bar" FROM "woo" GROUP BY "foo" HAVING "foo" > 4 AND "bim" in (SELECT "id" FROM "blah")`);
    });
};